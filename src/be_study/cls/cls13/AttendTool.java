...
public class AttendTool {
	...
	public void powerOn() { // 전원 켜는 기능
		this.isPowerOn = true; // 전원켜진 상태 값 -> true -> 켜진 상태
		//		필요한 로직 코드
	...
	public boolean checkAttend(Card card) {
		...
		System.out.printf("%s 소유의 %s 출결 확인\n", card.owner, card.brand);
	...
	//	출결 체크 결과 -> 기준코드	1:출근성공		2:실패	3:퇴근성공		4:조퇴처리		5:출근지각
	public int checkAttend2(Card card) {
		//		로직
		//		지금시간, 카드누누꺼, 과정/회사 등록, 이전에 출근이 찍혀있느냐..

		System.out.printf("[checkAttend2] %s 소유의 %s 출결 확인\n", card.owner, card.brand);
